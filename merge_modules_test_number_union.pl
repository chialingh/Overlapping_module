...
#open(IN, "/home/clhuang/lab/causal_modules/results/2012NOV06/modules/0p2/list.txt");
...
while(my $ff=<IN>){
	...
	my $module = "/home/clhuang/lab/causal_modules/results/2012NOV06/modules/0p2/$ff.txt";
#	$ff =~ s/\.txt//;
	chomp $ff;
	open(INF, "$module");
	while(my $g=<INF>){
...
my $aa = scalar keys %module_gene;
print "$ARGV[1]\t$aa\n";

=head
# print results
open(OUT, ">Merged_union_pathway_0p2_$cutoff.txt");
